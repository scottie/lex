...
bool LEXDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule LEXDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t LEXDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t LEXDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t LEXDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t LEXDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool LEXDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void LEXDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void LEXDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void LEXDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void LEXDGP::createParamsInstance(){
...
dev::Address LEXDGP::getAddressForBlock(unsigned int blockHeight){
...
void LEXDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void LEXDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void LEXDGP::parseStorageOneUint64(uint64_t& value){
...
void LEXDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule LEXDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
