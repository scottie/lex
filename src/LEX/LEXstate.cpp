...
LEXState::LEXState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = LEXState::openDB(_path + "/LEXDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

LEXState::LEXState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute LEXState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, LEXTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createLEXAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            LEX::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> LEXState::vins() const // temp
{
...
void LEXState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* LEXState::vin(dev::Address const& _a) const
{
    return const_cast<LEXState*>(this)->vin(_a);
}

Vin* LEXState::vin(dev::Address const& _addr)
{
...
// void LEXState::commit(CommitBehaviour _commitBehaviour)
// {
...
void LEXState::kill(dev::Address _addr)
{
...
void LEXState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address LEXState::createLEXAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void LEXState::deleteAccounts(std::set<dev::Address>& addrs){
...
void LEXState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void LEXState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
