...
public:
    ...
    unsigned int nBits;
    unsigned int nNonce;
    uint256 hashStateRoot; // LEX
    uint256 hashUTXORoot; // LEX
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot  = uint256(); // LEX
        hashUTXORoot   = uint256(); // LEX
    }
    ...
    CBlockIndex(const CBlock& block)
    {
        ...
        nBits = block.nBits;
        nNonce = block.nNonce;
        hashStateRoot  = block.hashStateRoot; // LEX
        hashUTXORoot   = block.hashUTXORoot; // LEX
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits = nBits;
        block.nNonce = nNonce;
        block.hashStateRoot  = hashStateRoot; // LEX
        block.hashUTXORoot   = hashUTXORoot; // LEX
        return block;
    }
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        ...
        if (this->nVersion > SC_BLOCK_VERSION) {
            READWRITE(hashStateRoot); // LEX
            READWRITE(hashUTXORoot); // LEX
        }
    }
