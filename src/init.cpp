...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "LEX.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "LEXd.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in LEX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in LEX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           LEX (or specifically: darksend, instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in LEX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -anonymizeLEXamount=<n>     " + strprintf(_("Keep N LEX anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The LEX-Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that LEX is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LEXcore is shutting down."));
...
#endif
    // Make sure only a single LEX process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LEXcore is probably already running."), strDataDir));
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateLEX";
                ...
                boost::filesystem::path LEXStateDir = GetDataDir() / "stateLEX";

                bool fStatus = boost::filesystem::exists(LEXStateDir);
                const std::string dirLEX(LEXStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsLEXState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<LEXState>(new LEXState(dev::u256(0), LEXState::openDB(dirLEX, hashDB, dev::WithExisting::Trust), dirLEX, existsLEXState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::LEXMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(LEXStateDir.string());
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LEXcore") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart LEXcore to complete") << "\n";
    ...
    nAnonymizeLEXAmount = GetArg("-anonymizeLEXamount", 0);
    if (nAnonymizeLEXAmount > 999999) nAnonymizeLEXAmount = 999999;
    if (nAnonymizeLEXAmount < 2) nAnonymizeLEXAmount = 2;

    LogPrintf("LEX darksend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize LEX Amount %d\n", nAnonymizeLEXAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LEX+1000 == (.1LEX+100)*10
       10LEX+10000 == (1LEX+1000)*10
    */
