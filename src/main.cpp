...
#if defined(NDEBUG)
#error "LEX cannot be compiled without assertions."
#endif
...
std::unique_ptr<LEXState> globalState;
...
const string strMessageMagic = "LEX Signed Message:\n";
...
#                   endif
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = LEXDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = LEXDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            LEXTxConverter converter(tx, NULL);
            ExtractLEXTX resultConverter;
            if(!converter.extractionLEXTransactions(resultConverter)){
            ...
            std::vector<LEXTransaction> LEXTransactions = resultConverter.first;
            std::vector<EthTransactionParams> LEXETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(LEXTransaction LEXTransaction : LEXTransactions){
                sumGas += LEXTransaction.gas() * LEXTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, LEXTransaction.gasPrice());
                } else {
                    txMinGasPrice = LEXTransaction.gasPrice();
                }
                VersionVM v = LEXTransaction.getVersion();
                if(v.format!=0)
                ...
                if(LEXTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(LEXTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(LEXTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)LEXTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(LEXETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > LEXTransactions.size())
...
//#if 0
    if (pindex->nHeight >= Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // LEX
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // LEX
...
#if 0
    LEXDGP LEXDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setLEXSchedule(LEXDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//LEXDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//LEXDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//LEXDGP.getBlockSize(pindex->nHeight + 1);
...
///////////////////////////////////////////////////////////////////////////////////////// LEX
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                LEXTxConverter convert(tx, &view, &block.vtx);

                ExtractLEXTX resultConvertLEXTX;
                if(!convert.extractionLEXTransactions(resultConvertLEXTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertLEXTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertLEXTX.first, blockGasLimit);
                ...
                for(LEXTransaction& ltx : resultConvertLEXTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertLEXTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertLEXTX.first[k].from(), resultConvertLEXTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // LEX
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // LEX
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // LEX
            globalState->setRootUTXO(oldHashUTXORoot); // LEX
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    LEXTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<LEXTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path LEXDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(LEXDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(LEXDir.string(), std::ios::in | std::ios::out);
...
bool LEXTxConverter::extractionLEXTransactions(ExtractLEXTX& LEXtx){
    std::vector<LEXTransaction> resultTX;
    ...
    LEXtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool LEXTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool LEXTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
LEXTransaction LEXTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    LEXTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = LEXTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = LEXTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
