...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setLEXSchedule(LEXDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = LEXDGP.getBlockSize(nHeight);
    minGasPrice = LEXDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = LEXDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    LEXTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractLEXTX resultConverter;
    if(!convert.extractionLEXTransactions(resultConverter)){
    ...
    for(LEXTransaction LEXTransaction : LEXTransactions){
        txGas += LEXTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + LEXTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(LEXTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, LEXTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LEXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("LEXMiner : ProcessNewBlock, block not accepted");
    }
