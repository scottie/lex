...
static void NotifyLEXNodeUpdated(MasternodeManager *page, CLEXNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateLEXNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyLEXNodeChanged.connect(boost::bind(&NotifyLEXNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyLEXNodeChanged.disconnect(boost::bind(&NotifyLEXNodeUpdated, this, _1));
}
...
void MasternodeManager::updateLEXNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        BOOST_FOREACH(PAIRTYPE(std::string, CLEXNodeConfig) adrenaline, pwalletMain->mapMyLEXNodes)
        {
            updateLEXNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditLEXNode* aenode = new AddEditLEXNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CLEXNodeConfig c = pwalletMain->mapMyLEXNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    LEXNodeConfigDialog* d = new LEXNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyLEXNodes.erase(sAddress);
        walletdb.EraseLEXNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CLEXNodeConfig) adrenaline, pwalletMain->mapMyLEXNodes)
    {
        CLEXNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
