...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LEX);
    ...
    if (!settings.contains("nAnonymizeLEXAmount"))
        settings.setValue("nAnonymizeLEXAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLEXAmount = settings.value("nAnonymizeLEXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLEXAmount"))
        SoftSetArg("-anonymizeLEXamount", settings.value("nAnonymizeLEXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in LEX.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLEXAmount:
            return QVariant(nAnonymizeLEXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLEXAmount:
            nAnonymizeLEXAmount = value.toInt();
            settings.setValue("nAnonymizeLEXAmount", nAnonymizeLEXAmount);
            emit anonymizeLEXAmountChanged(nAnonymizeLEXAmount);
            break;
        case CoinControlFeatures:
