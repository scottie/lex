...
bool ToHash160(const std::string& strLEXAddress, std::string& strHash160)
{   
    CTxDestination LEXAddress = DecodeDestination(strLEXAddress);
    if(IsValidDestination(LEXAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&LEXAddress);
...
bool ToLEXAddress(const std::string& strHash160, std::string& strLEXAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strLEXAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToLEXAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToLEXAddress(tokenEvent.receiver, tokenEvent.receiver);
