        ...
        model(0)
{
    ...
    ui->LEXAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaLEXLabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "LEX(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "LEX(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(LEX)"<<"TOTAL COST(BTC)");
...
void tradingDialog::UpdaterFunction(){
    //LEXst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
    ...
    QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/LEX_BTC");
    return Response;
}

QString tradingDialog::GetOrderBook(){

    QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/LEX_BTC");
    return Response;
}
...
void tradingDialog::GetMarketHistory(){
    ...
    sendRequest1(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/LEX_BTC"), f);
}
...
QString tradingDialog::BuyLEX(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";

    QJsonObject stats_obj;
    stats_obj["Market"] = "LEX/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellLEX(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"LEX\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>LEX Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> LEX");
...
void tradingDialog::AskInfoInsertSlot(int row, int col){
    ...
    QString UnitsLEXString = ui->AsksTable->model()->data(ui->AsksTable->model()->index(row,1)).toString();
    ui->SellBidPriceEdit->setText(SellBidPriceString);
    ui->UnitsInputLEX->setText(UnitsLEXString);
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->LEXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(LEXSupply,'i',8) + "</span><b> LEX</b>");
    ...
    ui->LEXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(LEXDemand,'i',8) + "</span><b> LEX</b>");
...
void tradingDialog::ParseAndPopulatePriceChart(QString Response){
    ...
    derPlot->yAxis->setLabel("LEX Price in BTC");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
        case 0:    //buy tab is active
            ...
            f = std::bind(&tradingDialog::showBalanceOfLEXOnTradingTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "LEX" + QString("\"}"));
            ...
            sendRequest1("https://www.cryptopia.co.nz/api/GetMarketOrders/LEX_BTC", f);
        ...
        case 1: //Cross send tab active
            ...
            f = std::bind(&tradingDialog::showBalanceOfLEXOnSendTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "LEX" + QString("\"}"));
        ...
        case 5://show balance tab
            ...
            f = std::bind(&tradingDialog::showBalanceOfLEXOnBalanceTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "LEX" + QString("\"}"));
...
void tradingDialog::showBalanceOfLEXOnTradingTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->LEXAvailableLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfLEXOnSendTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->CryptopiaLEXLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfLEXOnBalanceTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->LEXBalanceLabel,*ui->LEXAvailableLabel_2,*ui->LEXPendingLabel, QString::fromUtf8("LEX"),Response);
    }
}
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputLEX->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("LEX");
    ...
    double AvailableLEX = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling LEX available balance
    QString responseA = GetBalance("LEX");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableLEX = ResultObject["Available"].toDouble();

    ui->UnitsInputLEX->setText(str.number(AvailableLEX,'i',8));
}
...
{
    double Quantity = ui->CryptopiaLEXLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableLEX,'i',8));
}
...
void tradingDialog::on_BuyLEX_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyLEX(Order,Quantity,Rate);
...
void tradingDialog::on_SellLEXBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputLEX->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellLEX(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_CSUnitsBtn_clicked()
{
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        if ( ((Quantity / x) - y) > 0 )
        {
            ...
            QString SellResponse = SellLEX(Order,y,x);
            ...
            QString SellResponse = SellLEX(Order,(Quantity / x),x);
            QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
            QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
            ...
            if (SellResponseObject["Success"].toBool() == false){
                ...
                if (ResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" LEX for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
                } else if (ResponseObject["Success"].toBool() == true){
                    QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" LEX for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                }
...
void tradingDialog::on_UnitsInputLEX_textChanged(const QString &arg1)
{
    CalculateSellCostLabel(); //update cost
}
